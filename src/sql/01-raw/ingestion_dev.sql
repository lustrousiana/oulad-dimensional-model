...
-- Target: catalog `ftw-week-07`, schema `01-raw-dev`
--
-- WHY A DEV SCHEMA
--   Bri's tables in `01-raw` are untouched by this file. Both schemas can
--   exist side by side and be diffed. To promote, find-and-replace
--   `01-raw-dev` -> `01-raw` 

-- ---------------------------------------------------------------------
-- 1. RUN CONTEXT
-- ---------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `ftw-week-07`.`01-raw-dev`
COMMENT 'Bronze dev sandbox. Mirrors 01-raw for review. Do not build Silver from here.';
...
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`._ingest_audit (
...
-- ---------- courses ----------
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.courses (
    code_module                STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.courses
FROM (
    SELECT
        code_module,
        code_presentation,
...
INSERT INTO `ftw-week-07`.`01-raw-dev`._ingest_audit
WITH src AS (
...
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.assessments (
    code_module         STRING,
    code_presentation   STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.assessments
FROM (
    SELECT
        code_module,
        code_presentation,
...
-- ---------- vle ----------
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.vle (
    id_site             STRING,
    code_module         STRING,
    code_presentation   STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.vle
FROM (
    SELECT
        id_site,
        code_module,
...
-- ---------- student_info ----------
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.student_info (
    code_module          STRING,
    code_presentation    STRING,
    id_student           STRING,
    gender               STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.student_info
FROM (
    SELECT
        code_module,
        code_presentation,
...
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.student_registration (
    code_module         STRING,
    code_presentation   STRING,
    id_student          STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.student_registration
FROM (
    SELECT
        code_module,
        code_presentation,
...
-- ---------- student_assessment ----------
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.student_assessment (
    id_assessment       STRING,
    id_student          STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.student_assessment
FROM (
    SELECT
        id_assessment,
        id_student,
...
tgt AS (
    SELECT COUNT(*)                                                   AS rows_loaded,
           SUM(CASE WHEN _rescued_data IS NOT NULL THEN 1 ELSE 0 END) AS rows_rescued
    FROM `ftw-week-07`.`01-raw-dev`.student_assessment
)
...
CREATE TABLE IF NOT EXISTS `ftw-week-07`.`01-raw-dev`.student_vle (
    code_module         STRING,
    code_presentation   STRING,
    id_student          STRING,
    id_site             STRING,
...
COPY INTO `ftw-week-07`.`01-raw-dev`.student_vle
FROM (
    SELECT
        code_module,
        code_presentation,
...
SELECT
    ...
    rows_rescued,
    status
FROM `ftw-week-07`.`01-raw-dev`._ingest_audit
...
-- ---------------------------------------------------------------------
-- 4. DIFF AGAINST BRI'S EXISTING TABLES
--    Run once before proposing the merge. Any non-zero delta is a
--    conversation to have with her, not a number to overwrite.
-- ---------------------------------------------------------------------
SELECT 'assessments'          AS dataset,
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.assessments)          AS bri_rows,
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.assessments)      AS dev_rows
UNION ALL
SELECT 'courses',
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.courses),
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.courses)
UNION ALL
SELECT 'vle',
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.vle),
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.vle)
UNION ALL
SELECT 'student_info',
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.student_info),
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.student_info)
UNION ALL
SELECT 'student_registration',
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.student_registration),
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.student_registration)
UNION ALL
SELECT 'student_assessment',
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.student_assessment),
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.student_assessment)
UNION ALL
SELECT 'student_vle',
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.student_vle),
       (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.student_vle);