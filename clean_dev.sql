...
-- Target: catalog `ftw-week-07`, schema `02-clean-dev`
-- Source: `01-raw-dev` (populated by ingestion_dev.sql)
--
-- WHY A DEV SCHEMA
--   Keeps cleaning transformations separate from production `02-clean`.
--   To promote, find-and-replace `01-raw-dev` → `01-raw` and
--   `02-clean-dev` → `02-clean`
-- ============================================================================

CREATE SCHEMA IF NOT EXISTS `ftw-week-07`.`02-clean-dev`
COMMENT 'Silver dev sandbox. Mirrors 02-clean for review.';
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.assessments (
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.assessments
SELECT 
    id_assessment,
    code_module,
    code_presentation,
...
FROM `ftw-week-07`.`01-raw-dev`.assessments;
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.courses (
    code_module STRING COMMENT 'Module code (aaa-ggg)',
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.courses
SELECT 
    code_module,
    code_presentation,
...
FROM `ftw-week-07`.`01-raw-dev`.courses;
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.student_assessment (
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.student_assessment
SELECT 
    sa.id_assessment,
    sa.id_student,
    sa.date_submitted,
...
FROM `ftw-week-07`.`01-raw-dev`.student_assessment sa
LEFT JOIN `ftw-week-07`.`02-clean-dev`.assessments a
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.student_info (
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.student_info
SELECT 
    id_student,
    code_module,
    code_presentation,
...
FROM `ftw-week-07`.`01-raw-dev`.student_info;
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.student_registration (
    id_student INT COMMENT 'Student identifier',
    code_module STRING COMMENT 'Module code',
    code_presentation STRING COMMENT 'Presentation code',
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.student_registration
SELECT 
    id_student,
    code_module,
    code_presentation,
...
FROM `ftw-week-07`.`01-raw-dev`.student_registration;
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.student_vle (
    id_student INT COMMENT 'Student identifier',
    code_module STRING COMMENT 'Module code',
    code_presentation STRING COMMENT 'Presentation code',
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.student_vle
SELECT 
    id_student,
    code_module,
    code_presentation,
...
FROM `ftw-week-07`.`01-raw-dev`.student_vle
GROUP BY 
    id_student,
    code_module,
    code_presentation,
...
CREATE OR REPLACE TABLE `ftw-week-07`.`02-clean-dev`.vle (
...
INSERT INTO `ftw-week-07`.`02-clean-dev`.vle
SELECT 
    id_site,
    code_module,
    code_presentation,
...
FROM `ftw-week-07`.`01-raw-dev`.vle;
...
SELECT 
    ...
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.assessments) AS clean_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.assessments) AS raw_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.assessments WHERE has_missing_date = TRUE) AS quality_flag_count

UNION ALL

SELECT 
    'courses' AS table_name,
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.courses) AS clean_count,
...
SELECT 
    ...
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.student_assessment) AS clean_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.student_assessment) AS raw_count,
    NULL AS quality_flag_count
...
SELECT 
    ...
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.student_info) AS clean_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.student_info) AS raw_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.student_info WHERE has_missing_imd_band = TRUE) AS quality_flag_count

UNION ALL

SELECT 
    'student_registration' AS table_name,
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.student_registration) AS clean_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw`.student_registration) AS raw_count,
    (SELECT COUNT(*) FROM `ftw-week-07`.`02-clean-dev`.student_registration WHERE completed_course = TRUE) AS quality_flag_count
...
SELECT 
    ...
    (SELECT COUNT(*) FROM `ftw-week-07`.`01-raw-dev`.student_vle) AS raw_count,
